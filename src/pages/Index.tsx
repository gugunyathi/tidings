...
const Index = () => {
  ...
  const generateTiding = async () => {
    ...
    // Optional: play an ominous sound here, for now relying on UI
    ...
    try {
      ...
      if (data?.tiding) {
        ...
        // Emulate Omen Rarity if probability is extremely high
        ...
        const tiding: AiTiding & { videoUrl?: string; rarity?: string } = {
          ...data.tiding,
        ...
        // Post-gen Haptics & Effect
        if (assignedRarity === "Omen" && navigator.vibrate) {
          navigator.vibrate([500, 100, 500, 100, 500]); // intense!
        } else if (navigator.vibrate) {
          navigator.vibrate(100);
        }
        
        toast.success(`New tiding generated by the Oracle. Rarity: ${assignedRarity}`);
        // Scroll to top
        setCurrentIndex(0);
        scrollToIndex(0);
      }
  ...
  return (
    <div className="fixed inset-0 bg-background z-50">
      ...
      {/* Fullscreen Generation Gacha Overlay */}
      {isGenerating && (
        <div className="absolute inset-0 z-[100] bg-black/90 backdrop-blur-md flex flex-col items-center justify-center animate-omen-glow">
          <div className="animate-omen-shake relative flex flex-col items-center">
            <Sparkles className="w-20 h-20 text-oracle drop-shadow-[0_0_20px_rgba(255,215,0,1)] mb-8" />
            <h2 className="font-display text-4xl text-white font-bold tracking-widest text-center animate-pulse">
              CONSULTING<br/>THE ORACLE
            </h2>
            <div className="mt-8 text-oracle font-body text-sm tracking-[0.3em] uppercase">
              Searching the ether...
            </div>
          </div>
        </div>
      ...
      <div className="absolute top-0 left-0 right-0 z-30 flex items-center justify-between p-4 bg-gradient-to-b from-background/80 to-transparent">
        <h1 className="font-display text-2xl font-bold text-white tracking-wide drop-shadow-[0_0_10px_rgba(165,220,255,0.8)] animate-pulse-glow">TIDINGS</h1>
